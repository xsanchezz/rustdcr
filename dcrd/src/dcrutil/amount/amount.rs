...
pub struct Amount(pub(super) i64);
...
pub fn new(amount: f64) -> Result<Amount, super::AmountError> {
    if amount.is_nan() || amount.is_infinite() {
        return Err(super::AmountError::InvalidCoinAmount);
    }

    Ok(round(amount * super::constants::ATOMS_PER_COIN))
}
...
impl Amount {
    ...
    pub fn to_unit(&self, denom: Denomination) -> f64 {
        return self.0 as f64 / 10.0f64.powi(denom.precision() + 8);
    }
    ...
    pub fn format(&self, denomination: Denomination) -> String {
        return format!("{} {}", self.to_unit(denomination), denomination);
    }
